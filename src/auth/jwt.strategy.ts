...
export class JwtStrategy extends PassportStrategy(Strategy) {
    ...
    constructor(
        ...
        // UserRepository를 주입한 이유는
        // 토큰이 유효한지 확인한 다음에 username 기준으로 DB에서 user info를 가져와야 하기 때문이다.
        // 원래라면 private위에 @InjectRepository(UserRepository) 값을 넣어야 하나 버전 변경으로 사용 방법도 같이 바뀌었다.
    ) {
        super({// 부모 컴포넌트를 사용하기 위한..
            secretOrKey: 'Secret1234', // auth.module.ts의 secret: 'Secret1234'은 토큰 생성용
